<template>
  <header
    class="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="relative flex-1 max-w-md mx-8" role="search">
          ...
          <!-- Search Results -->
          <div
            v-if="_searchResults.length > 0"
            class="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 max-h-80 overflow-y-auto z-50"
            aria-live="polite"
        ...
        <nav id="navigation" class="hidden md:flex items-center space-x-8">
          <NuxtLink
            ...
            :aria-current="
              $route.path === `/blog/${category.toLowerCase()}`
                ? 'page'
                : undefined
            "
          >
            {{ category }}
            <span
              class="absolute -bottom-1 left-0 w-0 h-0.5 bg-purple-600 transition-all duration-200 group-hover:w-full"
            />
          </NuxtLink>
